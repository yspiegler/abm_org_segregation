...
run_sim <- function(org_structure = c(1,3,9), n_orgs = 50, prop_a_t1, prop_a_t2, prop_a_t3, 
  ...
  for (i in 2:n_iterations ) {
    ...
    mb <- microbenchmark(agents <- move_agents(agents, 3, have_prerefernce = T)) ### HERE YOU MAKE SURE PREF = TRUE. FALSE WILL BE RANDOM CHOICE
...
run_sim_batch <- function(n_sims, prop_a, prop_b) {
  ...
  for (i in 1:n_sims) {
    ...
    out <- run_sim(org_structure = c(1,3,9), n_orgs =  50, prop_a_t1 = prop_a, prop_a_t2 = prop_a, prop_a_t3 = prop_a, 
                   ...
                   prop_empty = 0.1, exact_prop = TRUE, agent_preferences = FALSE)
...
run_sim_batch(n_sims = 1, 0.5, 0.5)
