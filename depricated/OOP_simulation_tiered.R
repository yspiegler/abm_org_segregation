...
Organization <- R6Class(
  ...
  public = list(
    ...
    initialize = function(id, structure = c(1, 3, 9)) {
      if(length(structure)!=3 | !is.numeric(structure)) {
        print("Problem with the structure attribute. Structure needs to be a vector with three numbers - the number of agents in tier 1, 2, and 3")
        stop()
      }
      private$id <- id
      private$agents <- list(t1 = vector("list", structure[1]), 
                             t2 = vector("list", structure[2]), 
                             t3 = vector("list", structure[3]))
    },
